...
import com.mentor.action.CLGatepassAction;
...
public class CLGatepassImpl {

	// =================================get distillery
	// details===========================

	public String getDetails(CLGatepassAction ac) {
	...
	public ArrayList getSeasonDetails(CLGatepassAction ac) {
	...
	public ArrayList licListFL2Impl(CLGatepassAction act) {
	...
	public ArrayList displaylistImpl(CLGatepassAction act) {
		...
		try {
			...
			selQr = " SELECT a.package_name, a.package_id,c.vch_lic_no, a.duty, a.adduty, c.int_brand_id, b.brand_name,coalesce(c.tnt,'X') as tt , c.tnt ,"
					...
					+ " distillery.boxing_stock_19_20 c, distillery.box_size_details d  "
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				CLGatepassDT dt = new CLGatepassDT();
	...
	public String licenceeDetail(CLGatepassAction act) {
	...
	public boolean  etin(String casecode,CLGatepassAction act) {
		...
		try {
			...
			String query = " SELECT distinct b.code_generate_through FROM distillery.cl2_stock_trxn_19_20 a, distillery.packaging_details_19_20 b "
	...
	public String saveMethodImpl(CLGatepassAction act) {
		...
		String gatepass = String.valueOf(act.getDistId()) + "-2019-20-"+ act.getVch_from() + "-" + seq;
		int cases = 0;
		int totalBottles = 0;int indentbox = 0;
		double dutyAddDuty=0.0;
		...
		try {
			...
			insQr = " INSERT INTO distillery.gatepass_to_manufacturewholesale_cl_19_20( "
					...
					" gross_weight, tier_weight, net_weight,tnt) "
					+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?) ";
			...
			if (tok1 > 0) {
				...
				for (int i = 0; i < act.displaylist.size(); i++) {
					CLGatepassDT dt = (CLGatepassDT) act.getDisplaylist().get(i);

					if (dt.getDispatchBoxes() > 0 && dt.getInt_dispatch() > 0 && dt.getIndentbox()>0) {
						indentbox=dt.getDispatchBoxes();
						insQr1 = " INSERT INTO distillery.cl2_stock_trxn_19_20( "
						...
						if (tok1 > 0) {
							...
							String updtQr = " UPDATE distillery.boxing_stock_19_20 SET " +
									" int_dispatched=int_dispatched+'"+ dt.getInt_dispatch()+ "' " +
									" WHERE int_dissleri_id='"+ act.getDistId()+ "'  " +
									" AND int_brand_id='"+ dt.getInt_brand_id()+ "'  " +
									" AND int_pckg_id='"+ dt.getInt_pckg_id()+ "'  " +
									" AND vch_lic_no='"+ dt.getLicnobottling()+ "'  ";
						...
						for (int ii = 0; ii < act.getDisplaylist1().size(); ii++) 
						{
							...
							CLGatepassDT dt1 = 
									(CLGatepassDT) act.getDisplaylist1().get(ii);
				...
				if(tok1>0)
				{
					...
					String	 sql3 = " INSERT INTO distillery.duty_register_19_20( " +
							" int_id, int_distillery_id, date_crr_date, " +
							" vch_duty_type, int_quantity, int_value, vch_description,gatepass) " +
							" VALUES (?, ?, ?, ?, ?, ?, ?,?) ";


					ps4 = con.prepareStatement(sql3);
					ps4.setInt(1, getMaxIdDuty() + 1);
					ps4.setInt(2, act.getDistId());
					ps4.setDate(3, new java.sql.Date(System.currentTimeMillis()));
					ps4.setString(4, "DUTY_CL");
					ps4.setDouble(5, totalBottles);
					ps4.setDouble(6, dutyAddDuty);
					ps4.setString(7, "For Gatepass");
					ps4.setString(8, gatepass);
					tok1 = ps4.executeUpdate();

					//System.out.println(dutyAddDuty+"==dutyAddDuty==="+tok1);
	...
	public String getEmailDetails(CLGatepassAction ac) {
		...
		try {
			...
			String queryList = 	" SELECT a.officer_email " +
					" FROM public.district a, distillery.gatepass_to_manufacturewholesale_cl_19_20 b " +
	...
	public int maxId(CLGatepassImpl impl) {

		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(int_max_id) as id FROM distillery.gatepass_to_manufacturewholesale_cl_19_20 ";
		int maxid = 0;
		try {
			con = ConnectionToDataBase.getConnection();
			pstmt = con.prepareStatement(query);
	...
	public ArrayList getGatePassWholeSaleListCL(CLGatepassAction act) {
		...
		String query = " SELECT a.int_dist_id, a.vch_distillary_name, a.vch_distillary_address, a.vch_gatepass_no,coalesce(a.tnt,'X') as tnt, "
				...
				+ " distillery.gatepass_to_manufacturewholesale_cl_19_20 a  " +
				" WHERE a.int_dist_id="+ act.getDistId() + " ORDER BY a.int_max_id DESC";
		try {
			list = new ArrayList();
			con = ConnectionToDataBase.getConnection();
	...
	public ArrayList districtListImpl(CLGatepassAction act) {
		...
		try {
			String query = " SELECT districtid, description "
					+ " FROM public.district ORDER BY districtid ";
	...
	public boolean printReport(CLGatepassAction action) 
	{
		...
		try {
			...
			/*	"	SELECT DISTINCT a.int_dist_id, a.validtill,a.vch_distillary_name, a.gross_weight, a.tier_weight, a.net_weight, "
					...
					+ "	FROM distillery.gatepass_to_manufacturewholesale_cl_19_20 a,  "
					+ "	distillery.cl2_stock_trxn_19_20 b , "
					+ "	distillery.brand_registration_19_20 c , "
					+ "	 distillery.packaging_details_19_20 d, "
			...
			reportQuery = 	" SELECT  indent_no,vch_batch_no,int_dist_id, validtill,vch_distillary_name, gross_weight, tier_weight, " +
					...
					" FROM  distillery.CLGatepass("+action.getDistId()+",'"+action.getPrintGatePassNo().trim()+"' )";
			...
			if (rs.next()) {
				...
				CLGatepassDT dt1 = new CLGatepassDT();
	...
	// ------------------------------------------------
	public void saveExcelData(CLGatepassAction action) {
		String gatepass = "";
		int status = 0;
	...
	public int excelCases(CLGatepassAction act) {
		int id = 0;
		Connection con = null;
		PreparedStatement pstmt = null, ps2 = null;
		ResultSet rs = null, rs2 = null;
	...
	public int recieveCases(CLGatepassAction act) {
		int id = 0;
		Connection con = null;
		PreparedStatement pstmt = null, ps2 = null;
		ResultSet rs = null, rs2 = null;
		...
		try {
			 ...
			 */
			String query = 	" SELECT SUM(dispatchd_box) as dispatch_box FROM distillery.cl2_stock_trxn_19_20 " +
	...
	public void deleteData(CLGatepassAction action) {
		Connection con = null;
...
public boolean etin_check(String g) {
		...
		String q2 = "select b.f*count(b.code_generate_through) as count, b.code_generate_through as etin from" +
				...
				" (select dispatchd_box as f,int_pckg_id as p  from distillery.cl2_stock_trxn_19_20" +
	...
	public boolean updateFL3(CLGatepassAction act) {
		int save = 0;
		int save1 = 0;
		boolean val = false;
		PreparedStatement ps = null;
		...
		try {
			...
			for (int i = 0; i < act.getGetVal().size(); i++) {
				CLGatepassDT dt = (CLGatepassDT) act.getGetVal().get(i);
			...
			if (act.getGetVal().size() == save && save>0) {
				...
				query = " UPDATE distillery.gatepass_to_manufacturewholesale_cl_19_20 " +
	...
	public ArrayList getExcelData(CLGatepassAction action) {
		ArrayList list = new ArrayList();
	...
	public boolean getCascodeMatch(Date date1, String casecode,String etin, CLGatepassAction action)
	{
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
	...
	public boolean printDraftReport(CLGatepassAction action) {
	...
	public int maxIdSeq(CLGatepassImpl impl) {

		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = " SELECT max(seq) as id FROM distillery.duty_cancellation ";
		int maxid = 0;
		try {
			con = ConnectionToDataBase.getConnection();
			pstmt = con.prepareStatement(query);
	...
	/*public String cancel_gatepassImpl(CLGatepassAction act)
	{
...
String description="";
			...
			String selQr = 	" SELECT DISTINCT d.vch_description,a.int_dist_id,b.duty,b.addduty,b.dispatchd_box,(duty+addduty) as amount, " +
					...
					" FROM distillery.gatepass_to_manufacturewholesale_cl_19_20 a,  " +
					" distillery.cl2_stock_trxn_19_20 b, distillery.boxing_stock_19_20 c   ,distillery.duty_register_19_20 d " +
			...
			if (tok1 > 0) {
				...
				sql2 = 	" DELETE FROM distillery.gatepass_to_manufacturewholesale_cl_19_20 "+
			...
			if (tok1 > 0) {
				...
				sql3 = 	" DELETE FROM distillery.cl2_stock_trxn_19_20 "+
						" WHERE vch_gatepass_no='"+gatepass+"' " +
...
String description="";
			...
			if(tok1 > 0){
			  ...
			  tok1 =0 ;
				 ...
				 ps6 = con.prepareStatement(sql5);
					...
					//System.out.println("sql5------ggg----" + sql5);
	...
	public void getLicenseDetail(CLGatepassAction action)
	{
		...
		String query=" select vch_core_address,vch_firm_name from licence.fl2_2b_2d_19_20 " +
				"  where vch_licence_no='" + temp + "' and vch_license_type='CL2' ";
		try
		{
			con=ConnectionToDataBase.getConnection();
			stmt=con.prepareStatement(query);
	...
	public void saveCSV(CLGatepassAction action) throws IOException {
		Connection con = null;
		PreparedStatement stmt = null;
	...
	//-------------------------------------------------------------------------------------
	public ArrayList indentDetail(CLGatepassAction act) {
		ArrayList list = new ArrayList();
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public ArrayList displaylistImpl2(CLGatepassAction act) {
		ArrayList list = new ArrayList();
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		{ 
			...
			for (int ii = 0; ii < act.getDisplaylist1().size(); ii++) 
			{
				...
				if(dt.isSelected())
				{
					...
					indentno = " select sum(x.no_of_box) from ( SELECT  (m.no_of_box-coalesce(m.finalize_indent,0))no_of_box from  fl2d.indent_for_wholesale_trxn m where "+
 					" m.indent_no in ('"+dt.getIndentNo()+"')  and m.int_pckg_id="+dt.getPackageIdIndent()+" and m.int_pckg_id=c.int_pckg_id  and m.int_brand_id=a.brand_id_fk ";
					j++;
					}else{
			...
			selQr = " SELECT a.package_name, a.package_id,c.vch_lic_no, a.duty, a.adduty, c.int_brand_id," +
					" b.brand_name,coalesce(c.tnt,'X') as tt , c.tnt ,"
					+ " c.int_pckg_id, c.int_stock-c.int_dispatched as avlbottle, d.box_size, "
