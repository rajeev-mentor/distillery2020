...
import com.mentor.impl.LiquorVehicleAccidentCaseAECImpl;
 

public class LiquorVehicleAccidentCaseAECAction {

	LiquorVehicleAccidentCaseAECImpl impl = new LiquorVehicleAccidentCaseAECImpl();
	...
	public void view(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		LiquorVehicleAccidentCaseAECDt dt = (LiquorVehicleAccidentCaseAECDt) this.display_list
	...
	public void printDetail(ActionEvent e) {
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		LiquorVehicleAccidentCaseAECDt dt = (LiquorVehicleAccidentCaseAECDt) this.gatepass_list
				.get(uiTable.getRowIndex());
	}
	...
	LiquorVehicleAccidentCaseAECDt bopd;
	
	public LiquorVehicleAccidentCaseAECDt getBopd() {
		return bopd;
	}

	public void setBopd(LiquorVehicleAccidentCaseAECDt bopd) {
		this.bopd = bopd;
	}
	...
	public String finalizeData(ActionEvent e)
	{
		UIDataTable uiTable = (UIDataTable) e.getComponent().getParent()
				.getParent();
		LiquorVehicleAccidentCaseAECDt dt = (LiquorVehicleAccidentCaseAECDt) this.showDataTableList
				.get(uiTable.getRowIndex()); 
		try{
			if(dt.getDispatch_bottls()>0){
			if(dt.getDispatch_bottls()<=Integer.parseInt(dt.getShowDataTable_PlanNoOfBottling())){
			if(dt.getMaped_unmaped_flag().equals("M"))
			{
				
			    new LiquorVehicleAccidentCaseAECImpl().dataFinalize(this,dt);
			
			    
			   
			}else{
				new LiquorVehicleAccidentCaseAECImpl().finalizedUnmappedData(this,dt);
			...
			this.showDataTableList=impl.getDatacl(this);
				...
				FacesContext.getCurrentInstance().addMessage(
						...
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								" Bottles For Barcode should not be greater than planned no of bottles !! ",
								" Bottles For Barcode should not be greater than planned no of bottles !!"));
